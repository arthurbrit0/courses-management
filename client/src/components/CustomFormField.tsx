      ...
      case "select":
        return (
          ...
          >
            ...
            <SelectContent className="text-gray-400 w-full bg-customgreys-primarybg border-customgreys-dirtyGrey shadow">
              {options?.map((option) => (
                <SelectItem
                  key={option.value}
                  value={option.value}
                  className={`text-white cursor-pointer hover:!bg-gray-100 hover:!text-customgreys-darkGrey`}
                >
                  {option.label}
                </SelectItem>
              ))}
      ...
      case "file":
        ...
        return (
          <FilePond
            ...
            labelIdle={`Arraste e solte seus arquivos ou <span class="filepond--label-action">Navegar</span>`}
            credits={false}
          />
        );
      case "number":
